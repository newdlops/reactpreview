...
- global `fetch`는 compiler가 직접 호출을 계측하고, fetch 기반 third-party client의 HTTP(S), `/api`,
  `/graphql` 요청도 browser boundary에서 차단
...
요청별 Lorem/JSON override와 전역 Auto 설정은 해당 preview 탭에만 저장됩니다. 값을 바꾸면 page export를
