...
- 기본 Page Inspector는 workspace-bounded syntax/module resolution으로 증명된 actual ancestor export를
  ...
  않으며, 모드를 해당 패널에 고정한다.
- 작성자가 둔 dynamic import 경계는 full context의 local ESM chunk로 보존하고, Export Gallery의 cold fast
  pass만 publication 왕복을 줄이기 위해 단일 ESM entry로 합친다. 정적 import 의미는 바꾸지 않는다.
...
3. runtime environment resolver가 명시적 setup, `.react-preview/setup.*`, Storybook preview 순으로
   ...
   Gallery의 cold fast pass는 custom setup만 유지하고 자동 Storybook 선택을 full 보강까지 미룹니다.
...
Page Inspector cold session은 독립 target을 먼저 노출하지 않고 `full` request로 실제 ancestor page 문맥을
첫 artifact부터 만듭니다. Export Gallery cold session만 reverse workspace 분석을 생략한 `fast` request로
target-reachable graph를 먼저 게시하며, runtime settlement 뒤 `full` request가 setup/props/global 문맥을
보강합니다. 한 번 full context가 성공한 session은 이후 revision에서 증분 full request 하나만 실행합니다.
...
`previewInspectorLayoutRuntimeSource`는 격리된 custom host와 Shadow DOM portal의 하단·좌측·우측 drawer 및
floating 배치만 소유합니다. drawer 경계와 floating 모서리는 pointer/방향키 resize를 제공하고 floating 이동
...
만들어 실제 ancestor와 target 바깥 sibling을 보존합니다. 전체 bounded report는 Inspector Console과 원래
...
`previewRuntimeHookInstrumentation`은 Page Inspector full build에서만 reached workspace의 project-like import,
same-module custom hook 및 `use-query-params` 호출을 분석합니다. destructuring, compared literal, required
property/call path와 bounded semantic name으로 fallback expression을 만들 수 있는 호출만 global Inspector
resolver로 감쌉니다. resolver는 원본 hook을 정확히 한 번 호출하고 non-nullish 값은 그대로 반환합니다.
non-thenable 예외 또는 정적으로 필수인 nullish 결과는 `Auto values`가 켜진 경우에만 per-source identity로
고정된 frozen tuple/object/primitive/no-op 값으로 바뀝니다. Suspense thenable과 boundary를 끈 경우는 원래
예외/값을 보존합니다. 우회 record는 source·원본 headline·근거·생성값과 함께 ephemeral `Fallbacks` registry와
Console warning에 남으며 backend/application truth로 취급되지 않습니다.
