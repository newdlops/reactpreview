...
- 기본 component mode의 parent slice는 target까지의 JSX 한 갈래만 합성하며 parent owner 함수와 sibling
  JSX는 실행하지 않는다.
- opt-in Page Inspector만 workspace-bounded syntax/module resolution으로 증명된 actual ancestor export를 root로 실행하며, 모드를
  해당 패널에 고정하고 전체 app entry나 업무 route를 추측하지 않는다.
- 작성자가 둔 dynamic import 경계는 local ESM chunk로 보존하고 정적 import 의미는 바꾸지 않는다.
...
4. export inventory가 editor AST에서 runtime default와 PascalCase named value를 소스 순서대로
   ...
   wrapper branch를 export별 가상 render-slice module로 연결합니다. Page Inspector 요청이면 default 또는
   첫 explicit export에서 시작해 실제 importable owner를 별도 reverse plan으로 찾습니다.
5. 가상 엔트리는 빈 전역 namespace → setup initialize → target import → 공통/내보내기별 props →
   ...
   컴포넌트를 제거하지 않습니다. 깨진 자동 Storybook setup은 추적된 setup graph에 모든 오류가 속할
   때만 격리된 새 build에서 setup 없이 한 번 재시도합니다. render slice module은 target props를 원래
...
7. transformer가 TS/TSX AST로 Vite `import.meta.glob`/`globEager`, Webpack `require.context`, 상대
   ...
   analyzer는 실제 호출부의 non-optional container/callable 요구만 stable fallback으로 변환합니다.
...
9. 첫 graph가 Router consumer는 포함하지만 provider는 포함하지 않았음을 증명하고 Router bridge가 아직
   없다면 컴파일러가 같은 target root로 최대 한 번 adaptive rebuild합니다. custom/Storybook setup이
   존재한다는 이유만으로 이 결정을 억제하지 않으며 setup/target graph의 실제 provider 근거는 nested
   Router를 막습니다.
10. esbuild가 `platform: browser`, `format: esm`, `splitting: true`, `write: false`, `jsxDev: true`로
    entry JS, 원래 dynamic import 경계의 `chunks/*.js`와 선택적 집계 CSS를 메모리에 만듭니다. nested lazy tree는
    export별 Suspense 아래에서 실행되며 정적 자식 import를 임의로 lazy component로 바꾸지 않습니다.
    development JSX source metadata는 런타임 component stack의 파일·위치 근거로 보존됩니다.
11. application 유스케이스가 성공한 번들만 artifact store에 넘깁니다.
12. store가 내용 해시 디렉터리에 entry, CSS와 검증된 상대경로 chunk를 모두 쓴 뒤 opaque entry URI와
    한 개의 ownership reference를 반환합니다. entry의 상대 import가 같은 revision의 chunk를 읽습니다.
13. 첫 성공은 전체 HTML을 설정하고, 이후 성공은 cache-busted entry ESM·선택적 CSS URI와 revision token을
    기존 웹뷰에 보냅니다. 웹뷰는 기존 root를 unmount하고 stylesheet과 module을 교체한 뒤 확인 메시지를
    반환합니다. module 교체 오류는 runtime 보고서로 남기고, 메시지 전달 실패나 확인 timeout에는 전체
    HTML 교체로 복구합니다. 확인 뒤 이전 생성물 lease를 반환합니다. 이 경로는 서버 없는
    rebuild/remount이며 React Fast Refresh state 보존을 제공하지 않습니다.
...
`PreviewProjectUsageCache`는 최초 요청에서 package 안의 authored JS/TS source 경로를 구문 분석용 정적
...
`.git`, 생성 output, `node_modules`, symlink를 순회하지 않습니다. 이 단계가 보존하는 것은 inert path,
...
literal만 자동 props로 사용합니다. `parentSlice/previewParentSlice`는 target occurrence에서 owner boundary까지
...
선택된 consumer path는 session dependency에 합쳐 hot reload를 유발합니다. positive 결과는 consumer의
크기와 수정 시각이 유지되는 동안 재사용하지만 해당 dirty editor snapshot이 있으면 즉시 다시 분석합니다.
새 source와 새 usage를 받아들이도록 package inventory와 negative 결과는 5초 뒤 만료됩니다. cache는
...
정적으로 증명된 ancestry, target/root 선택, highlight, picker, remount와 plain JSON props override만
...
target·ancestor path와 forward graph input을 dependency에 합쳐 어느 선택 파일을 편집해도 해당 Inspector
패널만 rebuild하고 override를 새 root에 다시 적용합니다.
...
`PreviewCompiler.compile(request)` port는 요청 단위 계약을 유지하지만 adapter instance는 위의 inert
package/usage cache를 소유합니다. esbuild build context나 실행된 application state는 보존하지 않습니다.
향후 성능 측정 결과가 필요하면 같은 port 뒤에서 `context.rebuild()` 세션을 별도로 도입할 수 있으며
application과 UI 계약은 바뀌지 않습니다.
...
않습니다. optional/computed/array/write/conflict/shadow/prototype path와 한도 초과는 fail closed하고 leaf
값이나 app 의미를 생성하지 않습니다.
...
구조만 제공합니다. 앱 route graph, Redux/Formik/Context leaf 값이나 props는 이름만 보고 만들지 않습니다.
...
Apollo·Formik·Redux·Router·Theme bridge가 실제로 선택한 경계 상태를 함께 표시합니다. React error
