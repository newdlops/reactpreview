...
Blockers 기본 화면은 `Current blocking path`, `Current blocker`, `Next action`과 현재 단계의 실제
condition/payload/Smart fill 편집기만 한 열로 보여줍니다. 전체 `Control & render flow`와 오른쪽
`Blocker Resolver`는 `Advanced · Show flow graph`를 눌렀을 때만 생성되므로, 평상시에는 복잡한 그래프를 해석하거나
숨겨진 graph layout 비용을 지불하지 않습니다. Advanced에서는 graph block을 선택해 active/dormant path,
exact/inferred 근거, component owner 경로, source와 앞·뒤 block을 확인할 수 있습니다. `−`, `100%`, `+`,
`Center`, `Fit`은 별도 Inspector 탭의 graph viewport만 움직이며 페이지 runtime을 다시 실행하지 않습니다. 좁은
Inspector에서는 graph와 Resolver가 세로로 전환되고 카메라·Locate 버튼, 관계/condition/payload 편집기가 폭에 맞춰
wrap되며 각 영역이 독립적으로 scroll됩니다.
...
이동하며, 실행 전 정적 근거만 있으면 `CURRENT FILE · STATIC`으로 구분합니다. Advanced 오른쪽의
`CURRENT FILE PATH`는 현재 위치와 다음 행동을 먼저 보여주며, 접이식 안내에서
`Locate → Trace → Resolve → Verify` 순서를 설명합니다. 실선 active
...
- VS Code 1.100 이상
- React 16.8 이상과 호환되는 ReactDOM이 설치된 대상 프로젝트
...
| React 모듈을 찾지 못함             | 선택 파일의 package 경계에 React와 호환 ReactDOM 설치가 끝났는지 확인  |
