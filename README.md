...
preview renderer는 선택한 source package에서 `react-dom/client`를 먼저 확인합니다. React 18 이상이면
`createRoot`를 사용하고, subpath가 없는 React 16·17 프로젝트에서는 같은 package root의 legacy `react-dom`을
불러 `render`/`unmountComponentAtNode` adapter로 전환합니다. 따라서 확장 자체가 사용하는 React 버전이 프로젝트의
module resolution을 대신하지 않으며, 모노레포에서도 선택 파일에 가장 가까운 React 설치를 기준으로 bundle합니다.
...
왼쪽 1차 탐색은 `Components`와 `Blockers` 탭으로 나뉩니다. Components는 HTML tag가 아니라 runtime React
component를 부모·형제·자식 계층으로 보여주고, Blockers는 workspace root부터 현재 파일까지의 JSX 렌더 흐름을
`component function → guard/condition → selected return JSX → child component` 순서로 보여줍니다. 오른쪽
component debugger는 선택 항목의 `Props`, `State`, `Source`, `Payload`를 component 소유 범위로 보여줍니다. 별도 탭을
...
일반 tree 행 클릭은 별도 Inspector 탭의 현재 가로·세로 스크롤을 유지합니다. `Pick on page`, Wireframe marker,
...
다른 `PAGE PATH` 또는 `File components` 비교를 안내합니다. 실제 blocker에는 `Fix next blocker`, 성공한 page에는
`Reveal current file` 행동을 제공합니다. tree 범례와 각 행의 고정 역할 label은 일반 `COMPONENT`, 선택 파일의 `CURRENT FILE`, 실행하지 않은
...
해당 blocker가 선택된 `Blockers` Render flow를 엽니다. Components tree의 기존 선택과 스크롤은 유지하며,
사용자가 `Reveal in Components`를 명시적으로 눌렀을 때만 같은 blocker의 owner 경로를 펼쳐 이동합니다. 같은
아이콘을 다시 눌러도 해당 flow card와 editor가 다시 열립니다. 아이콘의 접근 가능한 이름과 tooltip에는 blocker
identity가 남습니다.
...
왼쪽의 `Blockers (N)` 탭은 blocker와 렌더 문맥을 page root에서 target 방향의 debugger control-flow graph로
보여줍니다. 각 실제 component는 함수 진입과 선택된 return JSX로 나뉘며, 그 사이에 compiler가 관찰한 `&&`,
ternary, early-return, overlay visibility 및 `switch/case`를 판단 노드와 `true`/`false`/`case`/`default` 선으로
표시하고 갈라진 경로가 다시 만나는 join도 명시합니다. 실행 경로는 실선, 현재 선택되지 않은 경로는 점선으로
구분합니다. 현재 파일의 return 뒤에는 실제 mounted child 호출을 bounded depth로 이어 붙여 upstream caller와
downstream output을 한 흐름에서 비교합니다. `memo`, `forwardRef`, `compose`, `with…` 계열 HOC와
`component`/`as`/render prop으로 전달된 component는 전용 호출 노드로 남고, 정적으로만 확인한 entry/route/wrapper는
실행된 return으로 오인하지 않고 read-only context node로 표시합니다.

Blockers 화면은 왼쪽 `Control & render flow`와 오른쪽 `Blocker Resolver`로 나뉩니다. 그래프의 block을 선택하면
Resolver가 active/dormant path, exact/inferred 근거, component owner 경로, source와 앞·뒤 block을 표시하고 기존
condition/payload/Smart fill 편집기를 같은 선택에 연결합니다. `−`, `100%`, `+`, `Center`, `Fit`은 별도 Inspector
탭의 그래프 viewport만 움직이며 페이지 runtime을 다시 실행하지 않습니다. 오른쪽 경계는 drag해 크기를 조절할 수
있고 `Inspector`로 접으면 그래프가 남은 공간을 사용합니다. 좁은 Inspector에서는 graph와 Resolver가 세로로
전환되고 카메라·Locate 버튼, 관계/condition/payload 편집기가 폭에 맞춰 wrap되며 각 영역이 독립적으로 scroll됩니다.

현재 파일을 찾으려면 `Locate current file`을 누릅니다. 확장은 선택 export 이름만 비교하지 않고 current-file marker,
정확한 source path와 mounted function entry가 모두 일치하는 `CURRENT FILE` block을 선택하고 중앙에 놓습니다.
아직 그 export가 page branch에 마운트되지 않았으면 `CURRENT FILE BLOCKER` 또는 가장 가까운 target-path blocker로
이동하며, 실행 전 정적 근거만 있으면 `CURRENT FILE · STATIC`으로 구분합니다. 오른쪽의
`HOW TO LOCATE CURRENT FILE`은 `Locate → Trace → Resolve → Verify` 순서와 다음 행동을 설명합니다. 실선 active
edge를 root 방향으로 추적해 첫 `!` blocker를 Auto/Smart/명시 값으로 해결한 뒤 다시 Locate하면 다음 blocker 또는
mounted current-file entry로 진행합니다.
...
요청은 임의로 직렬화하지 않고 각 branch의 독립된 경로로 표시합니다. 판단 노드는 authored/effective 값을 함께
보여주고 compiler가 안전하게 계측한 boolean branch와 literal/default switch branch만 스위치로 바꿉니다. runtime/data
노드는 기존 payload/pass/retry editor를 선택한 그래프 아래에서 펼칩니다. 현재 단계를 해결하면 다음
predecessor-ready blocker로 자동 이동하고, 트리에서 사라진 blocker도 해당 preview session 동안 완료 이력으로
남습니다. 그래프 노드 선택 자체는 Components tree 선택을 덮지 않으며 `Reveal in Components`만 owner row로
이동합니다. 기본 Flow는 mounted target owner path와 정적
entry→route→page→target source/name 증거에 속한 blocker를 우선합니다. 다른 sibling page blocker는 삭제하지 않고
Components tree에 유지하며 Flow 상단에 supporting blocker 개수로 알립니다.
선택한 현재 파일 export가 정확한 최종 owner인 미해결 blocker는 그래프에서 `CURRENT FILE BLOCKER` 배지와
노란 시작선으로 강조되고 상단에 개수가 표시됩니다. 이 표시는 component 이름이나 target-only 상태로 추측하지
않으며, mounted selected export와 exact owner ID 및 일치하는 source가 모두 확인될 때만 적용해 현재 파일의 직접
blocker와 imported child·정적 inventory·주변 page blocker를 구분합니다.
...
점진적으로 확장합니다. 사용자 condition/payload 값은 이 target-guided DFS보다 항상 우선합니다. 더 이상 안전한
...
- `Blockers (N)` Render flow에서 component function, 관여한 조건, 선택된 return, child 호출과 blocker 선후행을
  확인하고 compiler-proven branch switch 또는 `Fix this first` 값 적용·재시도
...
- component debugger의 `Payload` 또는 해당 Blockers card에서 Provider 예외, 필수 nullish 값 또는 실제 object의
...
- reached TSX/JSX의 `condition && <Component />`, JSX 삼항식, ReactDOM Portal branch와 Modal 계열의
...
선택 export와 page path, `Page flow`/`File components` view, highlight/Wireframe 상태, 선택한 blocker flow 단계, props/payload override와 Auto
