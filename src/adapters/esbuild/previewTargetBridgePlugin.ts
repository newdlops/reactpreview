...
export interface PreviewTargetBridgePluginOptions {
  /** Exact app-root CSS requests injected only around a standalone render target. */
...
export function createPreviewTargetBridgePlugin(options: PreviewTargetBridgePluginOptions): Plugin {
  ...
  function loadTargetBridge(arguments_: OnLoadArgs): OnLoadResult {
    ...
    return {
      ...
      ...(selections.length === 0 || options.applicationStylesheetImports === undefined
        ? {}
        : {
            watchFiles: options.applicationStylesheetImports.map(
              (selection) => selection.importerPath,
            ),
          }),
    };
  }
...
function createTargetBridgeSource(
  ...
  applicationStylesheetImports: readonly PreviewApplicationStylesheetImportSelection[],
