...
왼쪽의 `Blockers (N)` 탭은 blocker와 렌더 문맥을 page root에서 target 방향의 단계형 Render flow로 보여줍니다.
각 실제 component는 함수 진입과 선택된 return JSX로 나뉘고, 그 사이에 compiler가 관찰한 `&&`, ternary,
early-return, overlay visibility와 runtime/data blocker가 배치됩니다. 현재 파일의 return 뒤에는 실제 mounted child
호출을 bounded depth로 이어 붙여 upstream caller와 downstream output을 한 흐름에서 비교합니다. 정적으로만 확인한
entry/route/wrapper는 실행된 return으로 오인하지 않고 read-only context node로 표시합니다.
...
요청은 임의로 직렬화하지 않고 같은 stage의 병렬 카드로 표시합니다. condition card는 authored/effective 값을 함께
보여주고 compiler가 안전하게 계측한 branch만 true/false/reset 스위치로 바꿉니다. runtime/data card는 기존
payload/pass/retry editor를 그래프 안에서 펼칩니다. 현재 단계를 해결하면 다음 predecessor-ready blocker로 자동
이동하고, 트리에서 사라진 blocker도 해당 preview session 동안 완료 이력으로 남습니다. 카드 선택 자체는 Components
tree 선택을 덮지 않으며 `Reveal in Components`만 owner row로 이동합니다. 기본 Flow는 mounted target owner path와 정적
