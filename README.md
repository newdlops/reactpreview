...
하나의 entry로 게시합니다. 자동 Storybook setup, convention watcher와 code-split full graph는 그 빠른 화면이
실제로 mount됐다는 browser 확인 뒤에만 application entry, parent/sibling, props 및 전역 근거와 함께
백그라운드에서 보강합니다. 빠른 pass가 실패하면 처음부터 full pass로 전환하고, 보강만 실패하면 이미 보이는
프리뷰를 유지한 채 Output warning으로 남깁니다. full context를 한 번 확립한 탭의 다음 hot rebuild는 두 pass를
반복하지 않고 증분 full context를 한 번만 실행합니다.
...
- 최초 fast pass의 단일 ESM output과, full context에서 원래 dynamic `import()` 경계를 유지하는 최대 2,048개
  로컬 output 및 nested lazy component용 export별 `Suspense` 경계; 일반 정적 자식 import의 실행 의미는
  임의로 lazy 변환하지 않음
