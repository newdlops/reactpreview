...
웹뷰에 들어오지 않습니다. 이 모드는 기본 component gallery의 안전한 pinpoint slice와도 분리됩니다.
...
cold preview는 현재 export에서 정방향으로 도달하는 component/style/library graph를 먼저 네이티브 esbuild로
게시합니다. 그 빠른 화면이 실제로 mount됐다는 browser 확인 뒤에만 application entry, parent/sibling, props와
전역 근거의 전체 정적 분석을 백그라운드에서 보강합니다. 빠른 pass가 실패하면 처음부터 full pass로
전환하고, 보강만 실패하면 이미 보이는 프리뷰를 유지한 채 Output warning으로 남깁니다. full context를 한 번
확립한 탭의 다음 hot rebuild는 두 pass를 반복하지 않고 증분 full context를 한 번만 실행합니다.
...
선택 export, highlight 상태와 props override는 해당 웹뷰 탭에만 저장됩니다. 소스, 선택된 ancestor 또는
entry/lazy/route 경로 근거가 바뀌면 같은 패널이 서버 없는 ESM/CSS hot reload를 수행하고 override를 다시
적용합니다.
...
- 일반 component preview는 대상에서 가장 가까운 `package.json`을 모노레포 package 경계로 선택하고,
  ...
  module/entry/import fact는 변경된 파일만 다시 읽고 같은 package의 여러 탭과 재빌드가 공유합니다. opt-in
  Inspector의 reverse index만 bounded workspace
...
- 전체 앱 entry의 **실행**, 인증 bootstrap, 실제 API client나 backend 실행. 단, entry 위치와 render chain은
