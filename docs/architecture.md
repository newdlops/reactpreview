...
`previewInspectorBlockerUiRuntimeSource`는 condition에 더해 render-only hook fallback, no-network data request와
target-local contained error를 source path/line이 가장 가까운 mounted component 아래에 붙입니다. Context용 inert
route node는 owner 후보에서 제외하므로 blocker가 정적 경로 설명에 잘못 매달리지 않습니다. hook blocker의
