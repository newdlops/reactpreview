...
| `adapters/node`    | lock profile, 검증된 package 획득·layer와 전역 재사용   | Node 표준 라이브러리, tar, @yarnpkg/parsers  |
...
6. 일반 JS/TS import, package export, tsconfig alias와 symlink는 esbuild의 기본 resolver가 처리합니다.
   ...
   정확한 선언 package만 찾지 못했고 지원되는 npm lock evidence가 있으면 managed dependency adapter가
   global storage에 새 immutable environment를 게시하며, compiler는 이 새 identity로 전체 흐름을 정확히
   한 번 다시 시작합니다. 재시도 안에서 같은 resolve 오류가 나면 획득을 반복하지 않습니다.
...
## 관리 dependency environment

package resolver는 project-local installation, 현재 lock profile의 검증된 managed layer, 호환되는 extension
seed 순으로 후보를 선택합니다. seed catalog는 React/ReactDOM/Scheduler의 exact 18.3.1/18.3.1/0.23.2 및
19.2.7/19.2.7/0.27.0 tuple을 선언적으로 보존합니다. local React pair의 한쪽이라도 해석되면 seed를 선택하지
않고, lock-proven layer가 같은 core slot을 제공하면 seed를 노출하지 않습니다. 남은 경우에만 manifest의
React/ReactDOM range와 호환되는 catalog row를 선택하며 두 range가 모두 없으면 19 row가 기본입니다. npm alias
source directory는 ordinary package identity로 복사되고 extension package byte digest가 seed identity에
포함되므로 버전 문자열이 같아도 byte가 바뀐 stale seed를 재사용하지 않습니다.

성공한 browser bundle에 실제 도달한 일반 public package는 nearest lockfile, manifest dependency map,
플랫폼을 포함한 profile에 content-hashed layer로 등록할 수 있습니다. layer manifest와 package별 SHA-256이
맞지 않으면 resolver에 노출하지 않으며, workspace source와 절대경로는 보관하지 않습니다. 같은 profile의 새
target이 추가 package에 도달하면 기존 layer를 바꾸지 않고 새 immutable layer를 더합니다. Extension seed는
VSIX에 이미 들어 있는 package만 global storage에 materialize하며 workspace write나 lock 없는 registry fetch를
허용하지 않습니다.

project-local 및 기존 managed resolution이 모두 실패할 때의 cold acquisition은 다음과 같이 제한됩니다.

1. esbuild의 정확한 `Could not resolve "bare-request"` diagnostic에서 package root를 정규화하고, 현재
   `package.json`에 선언된 dependency만 requirement로 인정합니다. 상대경로, URL, Node builtin과 선언되지 않은
   오타는 network 후보가 아닙니다.
2. nearest supported lockfile에서 요청 root와 transitive dependency closure를 계산합니다. npm
   `package-lock.json` v2/v3는 `packages` map의 exact URL과 SHA-512 integrity를, Yarn v1은 exact version,
   public npm/Yarn registry URL과 SHA-512 integrity를 사용합니다. Yarn Berry는 exact `npm:` resolution을
   고정하고 `registry.npmjs.org`의 exact-version metadata에서 tarball URL과 SHA-512 integrity를 받으며 Yarn
   cache checksum은 SRI로 해석하지 않습니다. Closure 전체에 이 근거가 없으면 아무것도 게시하지 않습니다.
3. extension worker는 bounded concurrency와 byte budget 안에서 tarball을 받아 integrity를 먼저 확인합니다.
   모든 archive를 list-only로 먼저 검사해 closure 전체를 40,000 entry와 256 MiB file payload 안에 고정한
   뒤에만 workspace 밖 global storage staging directory로 추출합니다. path traversal, symlink/hardlink와
   특수 entry를 거부하고 package name/version을 재검증합니다. Package manager와 package script는 호출하지 않습니다.
4. 검증된 tree를 profile별 immutable layer로 atomic commit한 뒤 새 environment identity로 compile 전체를
   한 번 재시도합니다. 취소, download, 검증, extract 또는 재빌드 실패에서는 부분 layer를 제거하고 원래
   resolve diagnostic을 유지합니다.

pnpm lock, private/custom registry, git dependency, Yarn workspace, `workspace:`/`file:`/`link:` package와
SHA-512 근거가 없는 entry는 이 network 획득 경로에 들어오지 않습니다. 정상적으로 설치된 local package와
기존 PnP resolution은 package manager 종류와 관계없이 먼저 사용하며, 이 제한은 설치 없는 복원에만
적용됩니다. Managed layer는 session bundle과 분리된 quota/LRU store에 ordinary `node_modules` layout으로 남아
같은 exact profile의 다른 workspace가 다시 사용할 수 있습니다.
...
callback으로 materialize합니다. modal 자동 reveal의 단일 boolean은 hot entry 전용 resolver prop map에만 두고
`authored/inferred → resolver automatic → user JSON` 순서로 합성합니다. 이 map은 webview state에 저장하지 않으며
같은 entry의 data/condition 갱신에서는 고정되어 open/close 재시도를 만들지 않습니다. 따라서 첫 target commit 전
실패와 callback-only props도 `{}`로 축소되지 않습니다.
...
API/GraphQL·hook/Context·target props required-path는 `PREVIEW VALUE`/`BLOCKER` 행, JSX 조건은 inline switch/choice
행으로 표시합니다. Router/Provider/Theme, target reachability와 condition 수렴은 읽기 전용 자동 상태이며
syntax/module/lifecycle 오류는 setup 후보로 재분류하지 않고 Console 진단으로 보냅니다. 이 projection은 자동
resolver나 trace를 삭제하지 않고 별도 blocker graph만 만들지 않습니다. Smart fill은 required-path evidence가 있는 hook에서는 빈 호환 root에 demanded leaf만 만들고, descriptor로
...
page-path batch는 같은 reachability key의 hook/data record만 바꾸고 이미 증명된 target-guided condition을
유지하며 revision과 remount를 한 번으로 합칩니다.
...
module만 TypeScript syntax tree로 읽습니다. logical-and terminal은 direct JSX뿐 아니라 고유한 `const` alias,
JSX 배열·삼항식·중첩 AND, `React.createElement`, bounded map/flatMap callback까지 보수적으로 증명하고, terminal
앞의 guard를 평가 순서대로 각각 전역 Inspector API resolver에 연결합니다. render 함수 밖 module initializer는
remount로 재평가할 수 없으므로 계측하지 않으며, 겹치는 condition range는 static outcome과 같은 가장 바깥 결정을
남깁니다. module당 128개, session당 512개로 제한하고 parser recovery는 fail closed합니다.
logical guard는 thunk로 한 번만 지연 평가합니다. override가 없고 평가가 정상이면 원래 primitive/object와 단락
평가를 그대로 보존합니다. 앞 guard를 강제로 연 뒤 `session.user` 같은 다음 접근이 실패하면 authored false switch로
등록하고 Console warning을 남겨 전체 render를 중단하지 않으며, 사용자가 그 switch를 켜거나 owner의 preview 값을 채워
진행할 수 있습니다. forced false/true에서만 Boolean 결과를 사용합니다.
...
ephemeral map에 두고 명시적인 사용자 override보다 낮은 우선순위로 적용합니다. shared revision은 건강한
authored root의 React key를 바꾸지 않고 오류 경계의 reset signal과 구독 render만 갱신합니다. 따라서 Router,
provider, portal과 modal local state는 자동 condition/data 보정 사이에 유지됩니다. commit-producing 자동
condition은 blocker trace identity와 연결된 작은 transaction으로 등록합니다. 다음 fatal 오류가 같은 일반
condition attempt에 귀속되면 해당 override를 rejected set으로 이동하지만 overlay visibility는 닫지 않고 그
안에서 발견된 data/component 오류를 별도 blocker로 유지합니다. persisted user override는 rollback 대상이 아니며
...
한 번의 commit에는 gate 하나만 바꾸고 다음 React commit에서 새로 도달한 hook/API consumer를 관찰하므로 탐색은
...
payload inventory에 합칩니다. target-local 오류 경계가 실패를 보유하면 DFS는 추가 gate를 진행하지 않고 해당
blocker의 값 갱신이 probe revision을 한 번 올릴 때만 재평가합니다. 이 상태와 자동 condition 결정은 hot
descriptor/page/export 변경 시 폐기됩니다.
...
override는 선택한 tree blocker 상세에서 해당 compiler-issued hook ID에만 적용됩니다. Auto pass는 override를
...
condition/data/runtime-fallback/target-reachability registry와 blocker trace는 UI와 분리된 자동 resolver 경계입니다.
Inspector는 이 내부 처리 순서를 별도 graph로 복제하지 않고 enriched component tree의 정확한 owner 아래에 각 control을
붙입니다. hook/API/props 값은 해당 `PREVIEW VALUE` 또는 `BLOCKER` 행의 상세 editor를 재사용하며, 자동 준비는
registry별 non-committing mutation으로 revision을 먼저 올린 뒤 persistence, UI notification과 remount scheduling을
한 번만 수행합니다.

`previewInspectorLogicalSwitchModelRuntimeSource`는 선택 export의 정적 render outcome과 실제 runtime condition
registry를 source path·line·column·전체 expression fingerprint로 합칩니다. logical-AND chain의 group/index/count를
유지해 앞 guard에서 short-circuit된 후행 조건도 `Not reached yet` tree switch로 만들며, runtime resolver가 도달한
뒤에만 편집 가능한 condition ID를 연결합니다. 같은 group의 switch는 평가 순서대로 owner component 아래에 놓고,
실제 mounted truthy child가 증명되면 그 component branch를 switch 아래에 중첩합니다. 따라서 component 경로 자체가
렌더링을 막은 Boolean을 설명하며 별도 blocker DAG, camera, pan/zoom 또는 next-step queue가 필요하지 않습니다.

`previewReactRenderOutcomes`는 선택 module의 component-shaped export를 함수/HOC wrapper 안쪽까지 해석하고,
early `if/else`, ternary, logical `&&`, literal/default `switch`가 만드는 최대 32개 JSX/empty/unknown 반환 결과를
immutable plain data로 기록합니다. 반환 결과가 graph node이고 그 결과까지의 condition conjunction이 edge이며,
scalar JSX attribute는 결과 분기로 오인하지 않습니다. `previewInspectorRenderOutcomeExpansion`은 이 target-local
결과에서 시작해 direct import, local alias, barrel, namespace component와 단순 lazy/dynamic import를 resolver 기반
DFS로 확장합니다. source/module/depth/component 예산과 exact module-export cycle key를 사용하고, 구현 component와
호출부 children을 안정적인 순서로 병합하며 실제로 읽은 source를 ancestor plan dependency에 더해 HMR을 유지합니다.

logical-AND normalizer는 좌·우 결합 문법을 동일한 평가 순서의 guard 목록과 terminal로 제한적으로 펼칩니다.
runtime instrumenter와 outcome analyzer가 이 목록을 공유하므로 각 guard는 하나의 source identity와 Boolean UI를
가지며, Resolver는 guard의 truthy/falsy 조합만 다른 outcome을 하나로 접습니다. render terminal 증명은 고유한
lexical alias, JSX 배열·삼항식, `React.createElement`, bounded map/flatMap callback까지만 따라가고 scalar 계산,
모호한 binding, spread와 임의 함수 호출은 계측하지 않습니다.

browser의 outcome adapter는 compiler condition과 static edge가 source path·line·column·전체 expression
fingerprint로 일치할 때만 runtime Boolean switch를 연결합니다. 다른 source나 독립 분기의 수동 override는 보존되고
계속 자동 추론보다 우선합니다. 정적 outcome의 logical-AND group은 tree switch 목록을 만들 때 한 번 인덱싱하며,
current-file locator는 실제 mounted target, 완성된 static corridor target, unmounted inventory 순으로 선택합니다.
중간에서 budget에 잘린 static prefix는 성공 경로나 편집 가능한 condition으로 취급하지 않습니다.
...
component-tree owner 경로를 펼치고 해당 control 행과 상세 editor를 선택한 뒤 별도 companion tab을 포커스합니다.
detail revision은 같은 marker를 재선택해도 editor와 one-shot tree reveal을 다시 동기화합니다. DOM/Fiber reference는
React state나 webview persistence에 들어가지 않습니다. 한 refresh는
...
표시하며, layout-only `header`와 `article` 경계는 sanitizer allowlist에 보존해 React가 만든 grid 구조가 자식
평탄화로 변형되지 않도록 하고 별도 project entry를 import하지 않습니다. renderer의 zero-payload reveal request는 snapshot/action과
분리해 검증한 뒤 이미 생성된 companion panel만 focus합니다. `previewInspectorLayoutRuntimeSource`는 preview에 남는
page wireframe style과 companion에서 재사용할 component-tree/details workbench CSS를 소유합니다. 내부 workbench의
왼쪽은 하나의 `Components` tree이고 오른쪽은 선택 component debugger와 Console로 나뉩니다.
`previewInspectorComponentDebuggerUiRuntimeSource`는 `Props`, `State`,
...
logical-AND switch와 기타 render control은 선택 export의 owner corridor를 별도 graph로 복사하지 않고 component
tree 안에 pseudo child로 유지합니다. 현재 파일 직접 blocker 강조는 선택 경로에서 가장 안쪽의 mounted
`currentFileExport` node와 source path가 정확히 대응할 때만 부여하므로 target-only inventory나 동명 component를
현재 파일 blocker로 승격하지 않습니다.
...
같은 tree selection은 좌표·runtime revision·hot-stable sequence만 담은 decoration 전용 메시지를 Extension Host로
보냅니다. 각 preview session의 `PreviewInspectorSourceDecoration`은 마지막 성공 graph로 path를 다시 승인하고,
이미 보이는 동일 URI provider editor에만 whole-line mark를 적용합니다. 문서를 열거나 focus하지 않으며 source
edit, 새 selection, panel dispose에서는 이전 mark를 제거해 pinned preview 사이에 표시 상태가 섞이지 않습니다.
`previewInspectorCompanionPaneResizeScript`는 sanitized snapshot이 교체될 때 Components/Details 사이에 local
separator를 다시 삽입합니다. 넓은 화면의 Components pane은 52%에서 시작하며 760px breakpoint 전후의
column/row ratio를 별도 webview state로 유지하고 CSS grid
...
선택한 `PREVIEW VALUE` 행의 자동 생성은 inference/usage-derived prop layer만 초기화하고 실제 parent/setup props는 보존합니다.
일반 Props 및 `Fix blocker` detail은 공통 `previewInspectorPropsUiRuntimeSource`를 사용합니다. 이 editor는 Smart
draft의 generated path와 no-op sentinel을 명시하고 `Smart fill props` 적용 시 현재 export input revision만 갱신합니다.
...
data payload 또는 retry 값을 적용하면 input/error reset revision만 갱신합니다. 건강한 component는 유지되고
사용자가 `Remount`를 명시했을 때만 target child instance epoch가 바뀌며 authored page root는 유지됩니다.
같은 blocker가 page wireframe에서 선택되면 component `selectedTreeNodeId`를 해당 tree control로 바꾸고 owner
ancestor를 펼친 뒤 companion을 focus합니다. Components, Details와 Console viewport는 각각 stable scroll key를
...
plain object/array의 누락 nullish leaf는 preview auto-data layer가 켜진 경우에만 per-source stable fallback으로
...
예외/값을 보존합니다. 우회 record는 source·원본 headline·근거·생성 path/value와 함께 ephemeral fallback
registry, 선택 owner의 Payload/blocker 상세와 Console warning에 남으며 backend/application truth로 취급되지 않습니다.
