...
11. esbuild가 `platform: browser`, `format: esm`, `splitting: true`, `write: false`, `jsxDev: true`로
    entry JS, 원래 dynamic import 경계의 `chunks/[hash].js`와 선택적 집계 CSS를 메모리에 만듭니다. 같은
    target/runtime plan은 native `context.rebuild()`가 parsed graph를 재사용하고 editor snapshot과
