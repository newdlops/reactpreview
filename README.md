...
remount합니다. callback prop은 `[Preview no-op function]`으로 표시되며 실제 render 경계에서만 inert 함수가 됩니다.
...
- 직렬화 가능한 boolean·number·string·array·plain object props를 JSON으로 적용·초기화하고, `Smart fill props`로
  type/사용처/부모 JSX/오류 path에서 만든 최소 prop 초안을 적용
