...
이 확장은 특정 애플리케이션을 에뮬레이션하지 않는 범용 코드 뷰어입니다. 활성 파일의 컴포넌트
export를 소스 순서대로 보여주며, 각 export에서 정적으로 도달하는 자식 컴포넌트·CSS·CSS Modules·asset과
라이브러리 코드를 esbuild의 실제 import graph로 재귀 수집합니다. setup이 없는 경우 실제 JSX 사용처를
역추적해 대상까지의 wrapper 한 갈래만 Virtual DOM recipe로 복원하고 부모 함수와 형제 UI는 실행하지
않습니다. self-contained UI와 안전한 정적
Apollo operation은 바로 렌더링합니다. 도달한 styled-components 소스가 하나의 실제 `theme`을 명시하면
...
skeleton으로 제공합니다. leaf 값·업무 상태·실제 route table·권한은 임의로 만들지 않으며, 필요한 경우
setup이나 작은 preview harness로 네트워크 없는 계약을 조립합니다.

부모와 형제까지 작성된 화면 문맥에서 확인해야 할 때는 별도의 opt-in Page Inspector를 사용할 수
있습니다. Inspector는 workspace 안의 실제 JSX 사용과 barrel/tsconfig alias를 역추적해 찾은 importable
ancestor export를 마운트하므로 모노레포의 형제 app package까지 포함한 부모 children, sibling, 조건부 UI와
event handler가 일반 페이지처럼 실행됩니다.
이 모드는 전체 앱 entry를 추측해 실행하는 기능이 아니며 기본 component gallery의 안전한 pinpoint
slice와도 분리됩니다.
...
3. 에디터에서 우클릭해 `Open Current React File in New Preview`를 선택하거나, 명령 팔레트에서
   `React Preview: Open Current React File in New Preview`를 실행합니다. 실제 부모 화면 문맥이
   필요하면 대신 `Inspect Current React File in Page Context`를 선택합니다.
...
Page Inspector는 Chrome Elements와 비슷한 선택·강조 경험을 React source 단위로 제공하는 별도 모드입니다.
...
- 정적으로 증명된 root-to-target ancestry를 확인하고 target 또는 실제 ancestor root를 선택
...
웹뷰 개발자 console warning에 남으며 `Retry` 또는 toolbar의 `Remount`로 해당 target만 다시 시도합니다.

선택 export, highlight 상태와 props override는 해당 웹뷰 탭에만 저장됩니다. 소스 또는 선택된 ancestor
dependency가 바뀌면 같은 패널이 서버 없는 ESM/CSS hot reload를 수행하고 override를 다시 적용합니다.
...
- 일반 component preview는 대상에서 가장 가까운 `package.json`을 모노레포 package 경계로 선택하고,
  최초 빌드에서 그 경계 안의
  authored source 경로와 실제 JSX prop/wrapper 사용을 구문만 읽어 정적 환경 인덱스로 만듭니다. 인덱스는
  같은 package의 여러 탭과 재빌드가 짧게 공유합니다. opt-in Inspector의 reverse index만 bounded workspace
...
- 대상 import 전에 안전한 전역 namespace와 프로젝트 setup을 준비하고 render 시 Provider를 조립합니다.
  ...
  `globalThis/window.name = importedBinding` 할당을 실행 없이 수집합니다. 둘이 가리키는 정확한 project
  wrapper export를 esbuild lexical inject로 먼저 연결하며, 그런 강한 근거가 없을 때는 실제 도달 graph에서
  자유 식별자로 확인된 동일 이름 설치 package만 최대 한 번의 adaptive rebuild로 제공합니다. local binding은
  덮어쓰지 않고 wrapper의 plugin·locale·helper 정체성을 bare package보다 우선합니다.
...
- 생성된 entry JS, 원래 dynamic `import()` 경계를 보존한 보조 JS chunk와 집계 CSS는 인라인 코드나
  `eval`이 아니라 VS Code global storage의 외부 로컬 리소스로 로드합니다. 도달한 CSS는 먼저 적용하고
  JS chunk는 브라우저가 해당 경계를 실행할 때 로드합니다.
- 첫 렌더 뒤의 성공 빌드는 서버나 포트 없이 cache-busted local ESM URI와 선택적 CSS URI를 기존
  웹뷰에 전달합니다. 웹뷰는 stylesheet을 교체하고 React root를 다시 마운트하며, module 교체 오류는
  상세 runtime 보고서로 표시합니다. 메시지 전달이 실패하거나 확인이 시간 안에 오지 않으면 전체 HTML
  교체로 복구합니다. 이는 state를 유지하는 React Fast Refresh가 아닙니다.
...
- opt-in Page Inspector에서 workspace-local 실제 JSX 사용과 named/wildcard barrel·consumer tsconfig alias를
...
- 원래 dynamic `import()` 경계를 유지하는 최대 128개 로컬 ESM output과 nested lazy component용
  export별 `Suspense` 경계; 일반 정적 자식 import의 실행 의미는 임의로 lazy 변환하지 않음
...
- 전체 앱 entry, 인증 bootstrap, 실제 API client나 backend 실행. 단, import-backed 전역 할당/ambient
  선언이 정적으로 증명한 한 module export는 entry 자체를 실행하지 않고 lexical bridge로 재현할 수 있음
...
합계 20 MiB까지 허용하며 최종 JS/CSS 출력은 32 MiB, 출력 파일은 entry/CSS를 포함해 최대 128개로
제한합니다. 정적 매크로 하나는 최대
...
| 일부 component만 placeholder 표시 | console warning을 확인하고 Inspector 값 수정 후 Retry/Remount         |
