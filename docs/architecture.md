...
4. bridge는 대상 파일의 기본 내보내기만 노출해 무관한 named export graph를 tree-shake합니다.
5. workspace source plugin은 실제로 도달한 dirty 파일을 overlay하고 `.js`/`.ts` 계열 프로젝트와
   dependency source를 정적 리소스 transformer에 전달합니다. symlink·alias·순환 import로 다시
   유입되는 동일 파일은 canonical path로 비교해 하나의 모듈로 합칩니다.
6. transformer가 TS/TSX AST로 Vite `import.meta.glob`/`globEager`, Webpack `require.context`, 상대
   template·문자열 연결식 dynamic import/require와 정적 `new URL(..., import.meta.url)`을 유한한
   명시적 import/loader map으로 바꿉니다.
7. asset plugin이 `?raw`, `?url`, SVG component convention과 가장 가까운 package의 `public` root
...
8. esbuild가 `platform: browser`, `format: esm`, `write: false`로 단일 JS와 선택적 CSS를 만듭니다.
9. application 유스케이스가 성공한 번들만 artifact store에 넘깁니다.
10. store가 내용 해시 디렉터리에 파일을 모두 쓴 뒤 opaque URI와 한 개의 ownership reference를
    반환합니다.
11. 요청한 세션만 최신 revision의 HTML과 의존 그래프를 반영하고, 교체된 생성물 lease를 반환합니다.
