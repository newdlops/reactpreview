...
프리뷰 준비 중에는 대상 확인, 프로젝트 분석, 컴포넌트 문맥 탐색, 정적 런타임 준비, 번들링, 필요한
lockfile package 획득, 로컬 산출물 게시, React 로딩의 실제 8단계를 표시합니다. package 획득 단계는
해석 실패 뒤 지원되는 정확한 npm lock 근거가 있을 때만 나타납니다. 최초 빌드는 전체 화면으로 진행상황을 보여주고, hot reload는
...
- VS Code 1.100 이상
- React 16.8 이상과 호환되는 ReactDOM이 프로젝트에 설치되어 있거나, npm `package-lock.json` v2/v3 또는
  Yarn v1/Berry lock에서 정확한 public npm package를 확인할 수 있거나, manifest range가 확장에 포함된
  versioned React 18/19 runtime catalog와 호환되는 대상 프로젝트. 선택 우선순위는 project-local, lock-proven
  managed runtime, extension seed 순서
...
SolidJS와 Lit처럼 별도 JSX compiler 또는 template renderer를 사용하는 파일은 React 컴포넌트로 변환하지
않습니다. target import, nearest manifest와 `jsxImportSource`가 해당 runtime을 명확히 가리키면 package를
잘못 설치하거나 page graph를 스캔하지 않고 필요한 전용 toolchain을 설명하는 호환성 진단을 표시합니다.
...
- 프로젝트 설치가 성공적으로 해석되면 그것을 항상 사용합니다. 성공한 bundle에서 실제 도달한 일반
  ...
  React/ReactDOM/Scheduler 18.3.1/18.3.1/0.23.2와 19.2.7/19.2.7/0.27.0 exact tuple은 versioned extension seed
  catalog에 포함됩니다. project-local React나 ReactDOM 중 하나라도 있으면 seed를 섞지 않고, lock-proven
  managed layer가 core runtime을 제공하면 그 exact layer가 seed보다 우선합니다. 그 뒤에만 manifest의 React와
  ReactDOM range에 함께 호환되는 tuple을 선택하며, 두 range가 모두 없으면 19 tuple을 기본으로 사용합니다.
  seed의 React subpath와 peer import는 active issuer에서 다시 해석해 React/Context singleton을 유지합니다.
  이 seed는 VSIX에 이미 포함된 byte를 검증해 global storage로 복사할 뿐 workspace를 수정하거나 lock 없는
  다른 package를 registry에서 가져오는 경로가 아닙니다.
- local package와 기존 managed layer에도 없는 선언된 bare package 때문에 build가 실패하면 다음 lockfile
  형식에서 요청 package와 정확한 transitive closure를 선택합니다.
  - npm `package-lock.json` v2/v3: `packages` map의 exact URL과 SHA-512 integrity
  - Yarn v1 `yarn.lock`: exact version, public npm/Yarn registry URL과 SHA-512 integrity
  - Yarn Berry `yarn.lock`: exact `npm:` resolution을 고정한 뒤 `registry.npmjs.org`의 exact-version
    metadata에서 받은 tarball URL과 SHA-512 integrity. Yarn cache checksum을 tarball SRI로 사용하지 않음
- extension worker는 검증된 public tarball만 내려받아 안전한 global storage staging directory에서 검사하고
  ordinary `node_modules` layout의 immutable layer로 atomic publish합니다. package manager와 lifecycle/install
  script는 실행하지 않고 workspace의 manifest, lockfile, `.yarn` cache와 `node_modules`에도 쓰지 않습니다.
  모든 tar는 쓰기 전에 list-only preflight하며 closure 전체를 최대 40,000 entry와 256 MiB file payload로
  제한합니다. gzip 해제 결과도 package 및 closure 단위의 절대 상한을 넘으면 추출 전에 폐기합니다.
  새 environment identity로 전체 compile을 정확히 한 번만 재시도하므로 같은 오류가 반복되어도 획득·빌드
  loop에 들어가지 않습니다.
- 지원되는 lock evidence나 SHA-512 integrity가 없거나 pnpm lock, private/custom registry, git dependency,
  Yarn workspace 및 `workspace:`/`file:`/`link:` package, symlink, native/install-script 산출물이 필요한
  package는 자동 network 복원 대상이 아니며 원래 resolve 오류를 보존합니다. 이미 프로젝트에 정상 설치된
  package와 기존 PnP resolver는 이 제한과 무관하게 local-first로 계속 사용합니다.
- 명시적 tsconfig alias의 소스가 없고 가까운 bounded marker가 자동 생성을 증명하면 generated UI를
  구조적 React placeholder로 표시합니다. symlink의 실제 경로가 workspace를 벗어나면 marker를 읽거나
  watch하지 않습니다. symlink된 workspace package가 선언한 `dist` export만 없을 때는 정확히 대응하는
  체크인 `src` JS/TS/CSS 한 개를 사용하며 package script는 실행하지 않습니다.
- `?collection=...` MDX catalog import는 frontmatter와 검색 metadata만 bounded하게 만들고 본문 및 본문
  import graph는 열지 않습니다. 직접 선택한 MDX 문서는 로컬 compiler fallback으로 렌더링하며 프로젝트의
  MDX/remark/rehype 설정은 실행하지 않습니다.
- 256개 이상의 direct re-export를 가진 `sideEffects: false` package root는 named import와 public deep
  export가 같은 실제 leaf임을 증명한 경우에만 필요한 leaf로 축소합니다. default/namespace/dynamic/모호한
  요청은 원래 package barrel을 유지합니다.
...
- Browserify가 주입하던 자유 `process`를 사용하는 브라우저 package를 위해 setup과 target graph 평가 전에
  ...
  가진 bounded object를 설치합니다. 가장 가까운 package의 고정 dotenv convention에서는 `NEXT_PUBLIC_`,
  `VITE_`, `REACT_APP_`, `PUBLIC_` key만 읽고 비공개 key와 변수 확장은 webview에 전달하지 않습니다. 누락된
  공개 URL key는 소유·열거 결과를 바꾸지 않는 `https://react-preview.invalid/` 값으로만 직접 읽기를
  보완하므로 그 key를 직접 truthiness로 검사하는 분기는 URL이 있는 쪽으로 진행됩니다. hot reload에서는
  같은 object를 재사용하며 Node filesystem, network, native binding이나
  process 제어 권한은 만들지 않습니다. project bootstrap이 `process/browser`를 정확히 연결한 근거가 있으면
  위 lexical bridge도 그 project package identity를 재사용합니다.
...
- CSS별 가장 가까운 package의 Tailwind v4 `@tailwindcss/postcss` 및 Tailwind v2/v3 PostCSS fallback;
  ...
  `node_modules` linker를 사용해야 하며, 감지 시 구체적인 warning과 원본 CSS를 유지. 선언만 있고 package가
  없는 snapshot에서는 해석 불가능한 root `@import "tailwindcss"`만 생략해 나머지 authored CSS를 보존
- 설치가 없는 Next 프로젝트가 manifest에 `next`를 선언하면 `next/image`, `next/link`,
  `next/font/google`의 시각 계약만 local render-only facade로 제공; 서버/RSC와 framework config는 실행하지 않음
- 직접 `.mdx`와 `?collection=`/`only=frontmatter` query의 bounded local compilation 및 metadata export
...
- 고정된 development flag와 가장 가까운 dotenv convention의 bounded `VITE_*` 값만 제공하는
  `import.meta.env`
...
- 프로젝트 config 기반 custom `envDir`/`envPrefix`, dotenv 변수 확장과 Redux leaf 값·앱 route
  graph·props 의미를 추측하는 모킹
...
확장은 텔레메트리, workspace source나 preview payload를 외부로 보내지 않습니다. 단, project-local 및
managed package 해석이 실패하고 지원되는 lockfile이 정확한 public package를 증명하면 extension worker가
public registry에 exact-version metadata(Berry만 해당)와 tarball을 요청합니다. 이 요청에는 source, token 또는
credential을 싣지 않지만 registry에는 통상적인 네트워크 metadata와 요청 package가 보일 수 있습니다. 웹뷰
자체의 network CSP는 계속 차단됩니다. Vite/Next/Webpack/MDX 설정과 package script는 실행하지 않습니다.
고정된 `.env*` convention은 구문만 읽고 브라우저 공개 prefix의 값만 변수 확장 없이 세션 번들에 직렬화하며,
비공개 key는 전달하지 않습니다. 이 공개 값은 다른 생성물과 함께 VS Code global storage의 세션 디렉터리에만
저장되고 확장 종료 시 삭제되며 사용자 프로젝트에는 결과 파일을 쓰지 않습니다. 별도의
`dependency-store/v3`에는 검증된 public package tree만 symlink, nested
미도달 package, 실행 shim과 민감 설정 파일을 제외하고 보관합니다. lock 획득 package는 SHA-512 integrity를
먼저 검증하고, 각 immutable layer는 package별 SHA-256을 다시 검증한 뒤에만 resolver에 노출합니다. workspace
source·경로·토큰은 저장하지 않습니다. 이 저장소는 512 MiB/24 layer LRU 한도로 확장 종료 뒤에도 유지됩니다.
보안 문제는 [보안 정책](SECURITY.md)에 따라 공개 이슈가 아닌 비공개 경로로 제보해 주세요.
...
| classic JSX에서 `React`가 없음     | 0.1.1122 이상인지 확인하고 custom JSX runtime이면 namespace를 setup에서 명시              |
| MDX query loader 오류              | 0.1.1122 이상에서 collection metadata fallback 경고와 직접 MDX 크기 한도를 확인           |
| Next layout의 `Invalid URL`        | 0.1.1124 이상에서 metadata 격리와 공개 dotenv 경계가 적용됐는지 확인                      |
| generated UI alias를 찾지 못함     | 생성 marker/README 또는 프로젝트 generation 결과가 있는지 Output에서 확인                 |
| workspace package `dist`가 없음    | 정확히 대응하는 체크인 `src`가 없거나 symlink 경계 밖이면 package를 먼저 build            |
