...
이 확장은 특정 애플리케이션을 에뮬레이션하지 않는 범용 코드 뷰어입니다. self-contained UI와 안전한
정적 Apollo operation을 바로 렌더링하며, 프로젝트 소유 `styled-components`에는 값 없는 구조적 theme,
React Redux에는 빈 정적 store를 자동 연결합니다. 디자인 값·업무 상태·route·권한은 임의로 만들지
않습니다. 정확한 화면 의미가 필요하면 setup이나 작은 preview harness로 앱 bootstrap과 네트워크를
제외한 계약만 조립할 수 있습니다.
...
- `styled-components`에는 누락된 토큰 접근과 함수 호출을 빈 CSS로 축약하는 구조적 ThemeProvider를,
  React Redux에는 상태를 변경하지 않는 빈 정적 store Provider를 프로젝트 패키지 인스턴스로 제공합니다.
...
- 프로젝트 styled-components를 사용하는 구조적 fallback theme과 setup별 exact theme/비활성화
- 프로젝트 React Redux를 사용하는 빈 inert store와 setup별 exact static state/비활성화
- Router 등 library-branded context 오류를 번들 실패와 구분하고 범용 harness/setup 안내
...
- Vite 사용자 환경 변수, `.env` 로딩과 의미를 추측하는 deep Redux state·라우터·props 모킹
...
Apollo 정적 응답, 구조적 theme, 빈 Redux state를 조정하거나 Router, 전역값과 props를 공급하는 방법은
...
| Theme token 스타일이 비어 있음    | setup의 `themePreview.theme` 또는 실제 내부 ThemeProvider를 사용    |
| Redux selector가 nested 값에 실패 | setup의 `reduxPreview.state`에 화면에 필요한 최소 정적 상태를 공급  |
| Router 또는 필수 props 오류       | 범용 진단에 따라 preview harness나 setup에서 정적 계약을 공급       |
