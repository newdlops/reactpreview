...
2. 활성 문서와 같은 workspace의 dirty source 문서를 불변 snapshot으로 캡처합니다.
3. 가상 엔트리가 프로젝트 React/ReactDOM과 default-only target bridge를 가져옵니다.
4. bridge는 현재 파일의 기본 내보내기만 노출해 무관한 named export graph를 tree-shake합니다.
5. snapshot plugin이 실제로 도달한 dirty 파일만 전용 namespace에서 반환하고, symlink·alias·순환
   import로 다시 유입되는 동일 파일도 canonical path로 비교해 하나의 모듈로 합칩니다.
6. asset plugin이 `?raw`, SVG URL과 SVG component convention을 framework config 실행 없이
   JavaScript module로 변환하며 나머지 지원 asset은 data URL loader가 처리합니다. resolver는
   읽기 전에 일반 파일 여부와 파일당 5 MiB·합계 20 MiB 제한을 검증합니다.
7. esbuild가 `platform: browser`, `format: esm`, `write: false`로 단일 JS와 선택적 CSS를 만듭니다.
8. application 유스케이스가 성공한 번들만 artifact store에 넘깁니다.
9. store가 내용 해시 디렉터리에 파일을 모두 쓴 뒤 opaque URI를 반환합니다.
10. 컨트롤러가 최신 revision만 HTML에 반영한 뒤 이전 생성물을 정리합니다.
