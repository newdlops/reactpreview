...
- workspace의 정적으로 import된 `use*Context` 호출은 호출부가 실제로 비옵셔널 역참조하는 plain object
  ...
  첫 정적 DOM을 허용하되 leaf 값과 업무 의미는 만들지 않고, 실제 Provider 반환값은 그대로 우선합니다.
...
- 렌더 실패 시 direct error headline과 실패 phase, target/export/setup/classification, parent render
  slice의 wrapper 수·complete/partial 상태, 자동 Apollo·Formik·Redux·Router·Theme 경계 상태, React
...
- 전체 앱 entry, 인증 bootstrap, 실제 API client나 backend 실행
...
| custom Context·필수 props 오류    | 보고서의 component stack·경계 상태 뒤 harness/setup 계약을 공급       |
