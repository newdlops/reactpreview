...
generated entry의 runtime diagnostic catalog는 thrown value의 직접 `message`만 분류합니다. React Redux,
Apollo, Router가 소유한 문구는 번들 성공 후 누락된 app-root context로 안내하고, stack path나 프로젝트
이름은 규칙 입력으로 사용하지 않습니다. theme 함수, props와 unknown context 오류는 모두 일반적인
project runtime setup으로 남겨 잘못된 자동 Provider를 암시하지 않습니다. 원래 오류는 크기를 제한해
같이 표시하므로 디버깅 정보도 보존됩니다.
