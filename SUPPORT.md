...
6. 동적 리소스 오류라면 패턴이 `./` 또는 `../`로 시작하는 리터럴인지, 탐색 범위가 README의
   안전 한도 안인지 확인합니다.
7. Theme/context/전역/필수 props 오류라면 `.react-preview/setup.tsx` 또는
   `reactPreview.setupFile`이 프로젝트 Provider 계약을 준비하는지 확인합니다.
...
아닙니다. 대상 코드가 앱 root 밖에서 실행됐을 때 필요한 state, route, theme 또는 props가 없다는
뜻입니다. 플러그인은 특정 프로젝트 값을 추측하지 않으므로 `.react-preview/setup.tsx`나 작은
`*.preview.tsx` harness에서 네트워크 없는 정적 계약을 제공하세요.
...
props·router·상태 모킹은 현재 지원 범위가 아닙니다. 명시적 setup module의 Provider와 props는
지원하며 Apollo Client의 구조적 정적 응답만 예외적으로 자동 제공합니다.
