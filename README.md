...
자체의 network CSP는 계속 차단됩니다. Vite/Next/Webpack/MDX 설정, package script와 `.env` 파일을 실행하거나 읽지 않으며 사용자
프로젝트에도 결과 파일을 쓰지 않습니다. 생성된 번들은 VS Code global storage의 세션 디렉터리에만 저장되고
확장 종료 시 삭제합니다. 별도의 `dependency-store/v3`에는 검증된 public package tree만 symlink, nested
