...
인스턴스의 ThemeProvider를 outer boundary로 사용합니다. 기본 theme은 알려진 token을 갖지 않는 깊은
구조 Proxy이며 nested token, callable helper와 primitive 변환을 빈 CSS/`0`으로 끝냅니다. 실제
Provider가 안쪽에 있으면 React context 우선순위로 대체되고, setup의 `themePreview.theme`은 exact
reference로 전달됩니다. 디자인 값이나 프로젝트 theme module은 탐색하지 않습니다.

`previewReduxBridgePlugin`은 target package의 `react-redux`만 optional resolve합니다. reducer, middleware,
프로젝트 store module이나 `redux` package를 불러오지 않고 stable plain `{}`를 반환하는 최소 store를
같은 package의 outer Provider에 전달합니다. dispatch는 action을 실행하지 않고 그대로 반환하며 state와
listener를 바꾸지 않습니다. setup의 `reduxPreview.state`는 exact reference로 사용되고, 내부 실제
Provider가 있으면 자동 경계보다 우선합니다. nested slice나 권한 값은 생성하지 않습니다.
...
자동 계층은 Apollo selection-shaped data, 값 없는 styled-components token과 빈 Redux root처럼 의미를
추측하지 않아도 되는 최소 구조만 제공합니다. Router, nested Redux state, 디자인 값이나 props는
이름만 보고 만들지 않습니다. 프로젝트 의미가 필요한 값은 setup이 소유하며, 계약과 예시는
[프로젝트 setup 가이드](project-setup.md)에 둡니다.

generated entry의 runtime diagnostic catalog는 thrown value의 직접 `message`만 분류합니다. 자동
경계를 비활성화했거나 지원되지 않는 package에서 발생한 React Redux·Apollo·Router 소유 문구는 번들
성공 후 누락된 app-root context로 안내하고, stack path나 프로젝트 이름은 규칙 입력으로 사용하지
않습니다. 정확한 theme 값, props와 unknown context 오류는 일반적인 project runtime setup으로 남깁니다.
원래 오류는 크기를 제한해 같이 표시하므로 디버깅 정보도 보존됩니다.
