...
11. esbuild가 `platform: browser`, `format: esm`, `splitting: true`, `write: false`, `jsxDev: true`로
...
13. store가 내용 해시 디렉터리에 entry, CSS와 검증된 상대경로 chunk를 모두 쓴 뒤 opaque entry URI와
