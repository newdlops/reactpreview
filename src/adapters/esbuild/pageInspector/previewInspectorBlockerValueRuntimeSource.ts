...
function copyPreviewInspectorBlockerValueForJson(value, state, depth = 0) {
  ...
  if (typeof value === 'function') {
    ...
    if (marker?.value === true) return PREVIEW_INSPECTOR_COMPONENT_VALUE_SENTINEL;
    const callResult = Object.getOwnPropertyDescriptor(
      value,
...
function materializePreviewInspectorRuntimeFallbackOverride(value, depth = 0) {
  ...
  if (
    !Array.isArray(value) &&
    Object.hasOwn(value, PREVIEW_INSPECTOR_CALL_RESULT_TEMPLATE_KEY) &&
