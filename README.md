...
`Target-only view` 중 하나로 설명하고 바로 실행할 `Fix next blocker`, `Reveal current file`, `Return to page` 행동을
제공합니다. tree 범례와 각 행의 고정 역할 label은 일반 `COMPONENT`, 선택 파일의 `CURRENT FILE`, 실행하지 않은
