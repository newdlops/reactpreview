...
배치하므로 badge, JSON, 긴 component/source 이름이 화면 밖으로 밀리지 않습니다.
...
해당 preview session 동안 완료 이력으로 남습니다. 기본 Flow는 mounted target owner path와 정적
...
렌더 성공으로 오판하지 않습니다. Page Inspector는 authored page root commit과 target boundary mount가 같은
렌더 corridor에서 모두 확인된 경우에만 `PAGE READY`로 판정하며,
...
`Smart fill page path`는 이 corridor에서 이미 관찰된 hook과 backend payload만 한 번에 최소 shape로 바꾸고,
앞선 DFS에서 증명한 로그인/session/permission branch 선택은 유지한 채 페이지를 다시 시도합니다.
...
- `Fix blockers (N)`에서 blocker 선행·후행과 병렬 branch를 확인하고 `Fix this first` 단계부터 값 적용·재시도
...
- `Fallbacks`에서 Provider 예외, 필수 nullish 값 또는 실제 object의 누락 leaf 때문에 보완된 hook, 원본 오류,
  source·소유 component·필수/생성 path·타입/사용처 추론 근거 및 `GENERATED RENDER VALUE`를 확인하고 tree
  blocker에서 최소 required shape의 `Smart fill minimum`, property가 채워진 사용자 JSON pass value 또는
  더 넓은 inferred fallback을 보존하는 `Auto pass`를 적용
- `Payloads`에서 관찰된 GraphQL/REST 요청, 타입 추론 근거와 실제 전달할 JSON을 확인하고 `Generate Lorem`,
  `Smart fill minimum`, `Use Auto`, `Apply JSON`, `Reset override`로 backend 없는 응답을 변경
...
- 정상 commit됐지만 target이 없는 `Path blocker`에서 자동 통과한 로그인/session gate와 downstream payload
  property를 확인하고 `Smart fill page path`, `Retry application path` 또는 `Render target directly`를 선택
...
- 실제 target-rooted esbuild graph 전체에서 React Router consumer와 provider import를 수집합니다.
  ...
  후보별 target-facing render path를 다시 판정합니다. 현재 React tree에 Router context가 없고 그 후보도
  Router를 소유하지 않을 때만 같은 프로젝트의 후보 지역 `MemoryRouter`를 추가합니다.
